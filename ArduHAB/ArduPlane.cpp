...
#define SCHED_TASK(func, rate_hz, max_time_micros) SCHED_TASK_CLASS(Plane, &plane, func, rate_hz, max_time_micros)
   // Specially for release mechanism
		//int alt_temp=0;
		//int maxalt=0;
...
void Plane::handle_auto_mode(void)
{
    uint16_t nav_cmd_id;
	//////////////////////////////////////////////////////////////////////////
	hal.rcout->write(CH_5,1100);  // Payload release | mission start
	//////////////////////////////////////////////////////////////////////////
...
void Plane::stratocacherRelease()
{
		...
		{
			hal.rcout->write(CH_8,1800);  // Casing release channel 
			delay(5000); //wait for 5 second 
			hal.rcout->write(CH_7,1800);  // make sure the channel | release stratocacher 
		}
		else 
		{
			hal.rcout->write(CH_8,1100);  // Casing lock
			hal.rcout->write(CH_7,1100);  // Stratocaher lock 
		}
}

void Plane::releasestate()
{
		...
		if(gps.location().alt<g.release_altitude*100 && descendpass)
		{
		...
		if(gps.location().alt<g.parachute_ejectAlt*100 && descendpass)
		{
			hal.rcout->write(CH_6,1100);    //Parachute eject
		}
		
		if(gps.location().alt>g.parachute_ejectAlt*100 && !descendpass)
		{
			hal.rcout->write(CH_6,1900);    //Parachute Lock
		}
		...
		//RC_Channel_aux::set_radio(RC_Channel_aux::channel_function(6),1666);//x 1000
			hal.rcout->write(CH_5,1900);   //Payload Release Lock
		}
...
{		
	   //////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		int first_alt=10000*100;					//first pump trigger at first altitude in meters
		int second_alt=16000*100;				//second pump trigger at second altitude in meters
		//float cur_alt=gps.location().alt;
		//int cur_alt=inertial_nav.get_altitude();
		int cur_alt = current_loc.alt;
		if(cur_alt>first_alt&&cur_alt<first_alt+5000*100)
		{
		//trigger pump one
		hal.rcout->write(CH_6,1800);
		hal.rcout->write(CH_9,1522);   //servo lock 1 open
		}
		else
		{
		hal.rcout->write(CH_6,1100);
		hal.rcout->write(CH_9,1370);   //servo lock 1 close default
	    }
		
		if(cur_alt>second_alt&&cur_alt<second_alt+5000*100)
		{
		//trigger pump two
		hal.rcout->write(CH_7,1800);
		hal.rcout->write(CH_10,1520);		//servo lock 2 open
		}
		else
		{
		hal.rcout->write(CH_7,1100);
		hal.rcout->write(CH_10,1672);		//servo lock 2 close default
		}
...
{      
	...
	if(cur_alt>alt_1&&cur_alt<alt_1+2000*100)
		{
		//trigger on sma phase 1
		hal.rcout->write(CH_8,1800);
		}
	/* else
		hal.rcout->write(CH_8,1100); */
	
	 else if(cur_alt>alt_2&&cur_alt<alt_2+2000*100)
		{
		//trigger on sma phase 2
		hal.rcout->write(CH_8,1800);
		}
/* 	else
		hal.rcout->write(CH_8,1100); */
	
	 else if(cur_alt>alt_3&&cur_alt<alt_3+2000*100)
		{
		//trigger on sma phase 3
		hal.rcout->write(CH_8,1800);
		}
/* 	else
		hal.rcout->write(CH_8,1100); */
	
	 else if(cur_alt>alt_4&&cur_alt<alt_4+2000*100)
		{
		//trigger on sma phase 4
		hal.rcout->write(CH_8,1800);
		}
	/* else
		hal.rcout->write(CH_8,1100); */
	
	 else if(cur_alt>alt_5&&cur_alt<alt_5+2000*100)
		{
		//trigger on sma phase 5
		hal.rcout->write(CH_8,1800);
		}
	else
		hal.rcout->write(CH_8,1100);
